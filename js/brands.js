...
//   brandSection.classList.add("brands", "row", "gy-5", "justify-content-center");
...
async function createBrandDetails() {
  const brandsData = await fetchBrandData();

  const brandDetail = document.createElement("div");
  brandDetail.classList.add("brand-detail", "p-5");
  container.appendChild(brandDetail);

  const backBtn = document.createElement("button");
  backBtn.classList.add("btn", "btn-secondary");
  backBtn.id = "back-btn";
  backBtn.innerText = "Back";
  brandDetail.appendChild(backBtn);

  const brandRow = document.createElement("div");
  brandRow.classList.add("row");
  brandDetail.appendChild(brandRow);

  // const brandDetailContainer = document.getElementById("brand-detail");
  // container.addEventListener("click", (event) => {
  //   const target = event.target;
  //   if (target.classList.contains("brand-img")) {
